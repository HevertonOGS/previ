...
- **Running tests while iterating**: While actively developing, run only the test file(s) you just created or changed (e.g. `npx nx test api --testPathPattern=spreadsheet-import`), not the full project suite. Only run the full suite (`npx nx test api` / `npx nx test web`) when you actually need that broader confirmation — e.g. right before considering a task done, or after a change that could have cross-cutting effects.
